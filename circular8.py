...
def txt_setup(size,text,color):
	...
	show_v_cont,show_v_pos = DrawText("Set Velocity as:",350,350)
	window.blit(show_v_cont,show_v_pos)
...
def rotate_planet(planet_x,planet_y,planet_orb_r,planet_period,linear_acc,angular_acc,linear_speed,angular_speed,angle_val,deltaT):
...
while run:
	...
	if first_flag:
		...
		true_planet_orb_r = planet_orb_r*10**9
		ang_v = (2*math.pi)/(planet_period*24*60*60)
		lin_acc = true_planet_orb_r*(ang_v**2)-((const_G*mass_sun)/(true_planet_orb_r**2)) #m/s^2
		ang_acc = -(2*lin_v*ang_v)/true_planet_orb_r #rad/s^2
		planet_x,planet_y,lin_acc,ang_acc,lin_v,true_planet_orb_r,ang_v,angle = rotate_planet(planet_x,planet_y,true_planet_orb_r,planet_period,lin_acc,ang_acc,lin_v,ang_v,angle,del_t)
		...
		if rect_collide(400,400,sun_r,planet_x,planet_y,planet_r):
		...
		if cont_but:
			# draw text box
			pg.draw.rect(window,pg.Color(box_color),(text_box[0],text_box[1],text_box[2],text_box[3]),1)
			# draw text
			window.blit(t,(int(text_box[0]+text_box[2]/2-t.get_width()/2),int(text_box[1]+text_box[3]/2-t.get_height()/2)))
		'''
